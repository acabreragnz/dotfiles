...
- **Usar `AskUserQuestion` por default para preguntas con opciones discretas** — clarificaciones de requerimientos, elecciones entre alternativas técnicas, decisiones de diseño con 2-4 paths claros. Evita la pregunta inline en texto cuando el formato de opciones aplica. **Why:** el usuario lo pidió explícitamente; las opciones estructuradas son más rápidas de responder (click vs typing) y dejan la decisión auditable. **How to apply:** si vas a preguntar algo y podés enumerar 2-4 opciones discretas con descripciones, usar la tool. Preguntas abiertas ("¿qué pensás de X?") o follow-ups conversacionales siguen siendo inline. Recordá: cada question del tool acepta máximo 4 opciones, y "Other" se agrega automáticamente.
...
- Cuando el usuario pida abrir un worktree, una tab, o lanzar un comando en Warp → invocar el skill `/warp-agent`.
