...
- **Chrome DevTools MCP — pageId:** el server está levantado con `--experimental-page-id-routing` (`~/.claude.json`), así que **todos los tools page-scoped aceptan `pageId`** — `click`, `evaluate_script`, `navigate_page`, `take_screenshot`, `take_snapshot`, `hover`, `fill`, `fill_form`, `press_key`, `type_text`, `wait_for`, `handle_dialog`, `upload_file`, `resize_page`, `close_page`, `drag`, `screencast_start`, etc. Flujo: (1) `new_page` con la URL — no devuelve `pageId` estructurado; (2) `list_pages` y parsear la línea de tu tab (formato `N: URL [selected]`) → `N` es tu `pageId`; (3) pasar `pageId` explícito en CADA tool call posterior. **Why:** sin `pageId` los tools caen al "selected page" global del server (estado compartido entre todas las invocaciones), y en uso multi-agente los subagentes se pisan — verificado: dos agentes paralelos sin `pageId` operan ambos sobre la última tab seleccionada; con `pageId` explícito se aíslan aunque el "selected" global cambie mid-flow. PR [#1022](https://github.com/ChromeDevTools/chrome-devtools-mcp/pull/1022) introdujo el routing tras flag (merged 2026-02-26); PR [#1777](https://github.com/ChromeDevTools/chrome-devtools-mcp/pull/1777) lo hará default. **How to apply:** crítico al lanzar subagentes paralelos contra Chrome MCP; en single-agent también conviene por hábito y porque previene races contra tabs que abra el usuario en paralelo.
