...
- **`ast-grep` para buscar patrones de código:** preferí `ast-grep` sobre `grep` cuando el patrón depende de la estructura sintáctica — JSX elements con props específicas, llamadas de función, declaraciones de tipo, decorators, imports, etc. Para texto plano (logs, paths, strings literales en docs/JSON) usá `rg` (ripgrep) — más rápido que grep y respeta `.gitignore` automáticamente (evita `node_modules`, `__generated__`, etc.). Usar `grep` solo si `rg` no está disponible. Limitación: no ve dentro de spread props (`{...obj}`) — para eso, tracear la fuente del spread manualmente. Para la sintaxis de reglas, invocar el skill `/ast-grep`.
