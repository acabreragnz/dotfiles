...
- **`~/.agents/skills/` y `~/.claude/skills/` son copias auto-mirrored, NO symlinks** — pese a la convención global "AGENTS.md canónico, CLAUDE.md symlink", para `skills/` el harness mantiene **archivos duplicados independientes** que se sincronizan en background. **Why:** ya pasó — asumí que eran archivos reales independientes basándome en `diff … && echo SAME` (que solo prueba contenido idéntico, no estructura). Hice `rm .claude/.../SKILL.md && ln -s .agents/.../SKILL.md .claude/.../SKILL.md` para "migrar a symlink", el auto-mirror corrió en paralelo, y el canónico terminó como self-symlink (`-> /home/.../.agents/.../SKILL.md`), causando `ELOOP` al editar. **How to apply:** (1) editar cualquiera de los dos paths funciona — el mirror sincroniza; (2) NUNCA hacer `rm` + `ln -s` entre estos dos árboles; (3) antes de asumir estructura usar `ls -la` (muestra symlinks), nunca solo `diff` (solo prueba contenido); (4) si ves un self-symlink `X -> X`, lo más probable es que el mirror se confundió — borrar ambos y reescribir con Write directo.
