...
- **Chrome DevTools MCP — pageId:** todos los tools page-scoped aceptan `pageId` (flag `--experimental-page-id-routing` activo). Flujo: `new_page` → `list_pages` y parsear el `N` de la línea de tu tab (`N: URL [selected]`) → pasar `pageId: N` en cada call siguiente. **Why:** sin `pageId` los tools usan el "selected page" global del server, y subagentes paralelos se pisan. **How to apply:** obligatorio en multi-agente; recomendado siempre porque previene races con tabs abiertas por el usuario.
