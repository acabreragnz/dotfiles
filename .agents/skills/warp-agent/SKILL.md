...
| **opencode** | `cd <worktree-dir> && opencode -m <provider/model> --prompt "<prompt>"` | `-m <provider/model>` (e.g. `github-copilot/gpt-5.4`) | not exposed via CLI flag |
...
xdotool key ctrl+shift+d && sleep 1 && \
  xdotool type --clearmodifiers '<command>' && xdotool key Return
```
...
(empty — fill with real-execution failures as they happen, with user approval)
